...
import com.t24.services.impl.FC01Tipo1NuevoImpl;
...
public class CuentaClienteNuevoImpl {
    private static final Logger LOG = LogManager.getLogger(CuentaClienteNuevoImpl.class);
    ...
    {       
        ...
        FC01Tipo1Nuevo fc01= new FC01Tipo1NuevoImpl();
...
//                         System.out.println(json); 
                       ...
                       for (int i = 0; i < doctypeArray.length; i++) {
                        ...
                        List<ComisionClienteBIZAGI> listComisionClienteBIZAGI =comisionclientebizagi.comisionclientebizagiQry(segmento, tipotransferencia, Constantes.ESBANCOLOCAL);  
                        ...
                        for (FCCuenta cyf : CuentayFirma) 
                        {
                            if(!cyf.getPortfolio().trim().equals(""))
                            {
                                ...
                                lstPrt.setWorkingBalinUSD(cyf.getWorkingBalUSD());
...
//                            System.out.println(json); 
                            ...
                            {                                
                                JOCliente.put("nombre", lpf.getNombre());
                                JOCliente.put("apelli",lpf.getApellidos());
                                for (FCDocIdentidad lpf1:listDoc)
                                {
                                    if (lpf.getIdClient().trim().equals(lpf1.getIdclie().trim()))
                                     {
                                            JSONObject JODocIdentidad=new JSONObject(); 
                                            JODocIdentidad.put("tipdoct24", lpf1.getId());
                                            JODocIdentidad.put("numdoc", lpf1.getDoctype());    
                                            JODocIdentidad.put("tipdocte", lpf1.getDoctypete());   
                                            JODocIdentidad.put("tipdoc", lpf1.getDoctypeib());            
                             ...
                             for (FCCuenta lpf:CuentayFirma)
                                     {   
                                        ...
                                        JOCuenta.put("moncta",lpf.getCurrency());
                                        ...
                                        estitun="";
                                            if (lpf.getRelationCode().equals(Constantes.T24_RELATIONCODE_SECUNDARY))    // 10: Secundary
                                                 {  
                                                     estitun="T";                                       
                                                 }
                                            else if (lpf.getRelationCode().equals(Constantes.T24_RELATIONCODE_POWEROFATHOLDER))  // 6: Power of Atorney Holder
                                                 {                                        
                                                     estitun="F";
                                                 }
                                            else
                                        ...
                                        JOCuenta.put("refer","");
                                        JOCuenta.put("trustee",Convierte.NullaVacio(lpf.getItlgCustType()));
                                        JOCuenta.put("signcondition",Convierte.NullaVacio(lpf.getSignCondition()));
                                        JOCuenta.put("signrelation",Convierte.NullaVacio(lpf.getSignRelation()));
